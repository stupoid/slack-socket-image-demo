...
def get_df(
    country: str, column: Literal["lifeExp", "pop", "gdpPercap"] = "lifeExp"
) -> pd.DataFrame:
    df = px.data.gapminder().query(f"country=='{country.capitalize()}'")
    return df[["year", column]]
...
def generate_plot(
    ...
    fig = px.line(
        df, x=df.columns[0], y=df.columns[1], title="Life expectancy in Canada"
    )
